...
# Allow imports from project root (agent/, rag/)
...
<style>
    ...
    .advisory-header { font-size: 1.3rem; font-weight: 700; color: #0F172A; margin-bottom: 0.5rem; }
    .advisory-sub { font-size: 0.9rem; color: #64748B; margin-bottom: 1.5rem; }
...
def do_the_sample(city, loc, ptype, area, beds, baths, floor, total, age, furnish):
    st.session_state.form_data = {
        ...
        "beds": beds, "baths": baths, "floor": floor, "total": total,
        "age": age, "furnish": furnish,
    }
...
with col_s1:
    if st.button("Load Gurgaon Sample"):
        do_the_sample("gurgaon", "dlf phase 1", "Apartment", 2400, 3, 3, 5, 12, 4, "Semi-furnished")
with col_s2:
    if st.button("Load South Bombay Sample"):
        do_the_sample("mumbai", "malabar hill", "Apartment", 3200, 4, 5, 18, 30, 3, "Furnished")
...
with col_results:
    if st.session_state.prediction_shown:
        ...
        if loaded_model is not None and dataset_features is not None:
            try:
                inp = pd.DataFrame([{
                    "city": u_city.lower().strip(), "location": u_loc.lower().strip(),
                    "property_type": property_type, "bedrooms": beds, "bathrooms": baths,
                    "area_sqft": area_sqft, "floor_num": floor_no, "total_floor": total_f, "age": age_v,
                }])
...
# ── AI Advisory Report section ─────────────────────────────────────────────
if st.session_state.prediction_shown:
    st.markdown("<div class='advisory-card'>", unsafe_allow_html=True)
    st.markdown(
        "<div class='advisory-header'>AI Investment Advisory</div>"
        "<div class='advisory-sub'>Powered by Llama 3.3 70B via Groq · RAG-grounded with live market data</div>",
        unsafe_allow_html=True,
    )

    # Unique key for this property combination to detect stale reports
